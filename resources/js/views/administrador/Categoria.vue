  ...
  <div class="app-shell">
    ...
    <aside class="sidebar">
      <router-link to="/" class="sidebar-logo">
        ...
        <div><div class="sidebar-logo-text">BioFund Admin</div></div>
      </router-link>
      ...
      <main class="content">
        ...
        <div class="cat-grid" v-else>
          <div
            ...
            :key="cat.id"
            class="cat-card"
            :class="{ inativa: !cat.is_active }"
          >
            <div class="cat-card-header">
              <div class="cat-icon-wrap" :style="{ background: (cat.color || '#52B788') + '22', borderColor: (cat.color || '#52B788') + '55' }">
                <svg width="22" height="22" fill="none" :stroke="cat.color || '#52B788'" stroke-width="1.7" viewBox="0 0 22 22">
              ...
              <div class="cat-actions">
                ...
                <button class="btn-icon-sm" @click="openSubs(cat)" title="Subcategorias">
                  <svg width="13" height="13" fill="none" stroke="currentColor" stroke-width="1.8" viewBox="0 0 14 14">
                    <path d="M2 3h10M2 7h7M2 11h4" stroke-linecap="round"/>
                  </svg>
                </button>
              </div>
            </div>
            ...
            <div class="cat-footer">
              ...
              <span class="cat-stat" v-if="cat.subcategories?.length">
                <svg width="12" height="12" fill="none" stroke="currentColor" stroke-width="1.6" viewBox="0 0 12 12">
      ...
      <div v-if="modalOpen" class="modal">
        ...
        <div class="modal-body">
          ...
          <div class="f-row">
            <div class="f-group">
              ...
              <input type="text" v-model="form.code" placeholder="Ex: FAU" maxlength="20" :class="{ 'f-err': fieldErrors.code }"/>
        ...
        <div class="modal-ft">
          ...
          <button class="btn-guardar" @click="guardar" :disabled="saving">
            <svg v-if="saving" class="spin-sm" width="14" height="14" fill="none" stroke="#fff" stroke-width="2.2" viewBox="0 0 16 16">
    ...
    <!-- ── MODAL SUBCATEGORIAS ── -->
    <transition name="fade">
      <div v-if="subsModal.open" class="modal-overlay" @click.self="subsModal.open = false"></div>
    </transition>
    <transition name="modal-pop">
      <div v-if="subsModal.open" class="modal modal-subs">
        <div class="modal-hd">
          <div>
            <h3>Subcategorias · {{ subsModal.cat?.name }}</h3>
            <p>Gerencie as subcategorias desta categoria.</p>
          </div>
          <button class="btn-close" @click="subsModal.open = false">
            <svg width="13" height="13" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 13 13">
              <path d="M2 2l9 9M11 2L2 11" stroke-linecap="round"/>
            </svg>
          </button>
        ...
        <div class="modal-body">
          ...
          <div class="sub-add-row">
            <input type="text" v-model="subsModal.newName" placeholder="Nome da subcategoria…" @keyup.enter="addSubcategoria"/>
            <button class="btn-add-sub" @click="addSubcategoria" :disabled="subsModal.saving">
              <svg width="13" height="13" fill="none" stroke="#fff" stroke-width="2" viewBox="0 0 13 13">
          ...
          <div class="sub-list" v-if="subsModal.list.length">
            <div class="sub-item" v-for="sub in subsModal.list" :key="sub.id">
              <template v-if="subsModal.editingId === sub.id">
                <input class="sub-edit-input" type="text" v-model="subsModal.editName" @keyup.enter="saveSubEdit(sub)" @keyup.escape="subsModal.editingId = null"/>
                <button class="btn-sub-save" @click="saveSubEdit(sub)">✓</button>
                <button class="btn-sub-cancel" @click="subsModal.editingId = null">✕</button>
              </template>
              <template v-else>
                ...
                <button class="btn-icon-sm" @click="startSubEdit(sub)" title="Editar">
                  <svg width="12" height="12" fill="none" stroke="currentColor" stroke-width="1.8" viewBox="0 0 14 14">
                    <path d="M9.5 2.5l2 2L4 12H2v-2L9.5 2.5z" stroke-linejoin="round"/>
                  </svg>
                </button>
                <button class="btn-icon-sm" @click="toggleSubActive(sub)" :title="sub.is_active ? 'Desactivar' : 'Activar'">
                  <svg width="12" height="12" fill="none" stroke="currentColor" stroke-width="1.8" viewBox="0 0 14 14">
                    <circle cx="7" cy="7" r="5"/><path v-if="sub.is_active" d="M5 7l2 2 3-3" stroke-linecap="round"/>
                    <path v-else d="M5 5l4 4M9 5l-4 4" stroke-linecap="round"/>
                  </svg>
                </button>
              </template>
            </div>
          </div>
          <p class="sub-empty" v-else-if="!subsModal.loading">Esta categoria ainda não tem subcategorias.</p>
          <div class="loading-row" v-if="subsModal.loading">
            <span class="spin"></span> A carregar…
          </div>
        </div>
      </div>
    </transition>
...
/* SIDEBAR */
.sidebar { width: 210px; flex-shrink: 0; background: var(--white); box-shadow: 2px 0 18px rgba(0,0,0,.07); display: flex; flex-direction: column; height: 100vh; position: fixed; top: 0; left: 0; z-index: 50; }
.sidebar-logo { display: flex; align-items: center; gap: 9px; padding: 18px 16px 16px; border-bottom: 1px solid rgba(0,0,0,.06); text-decoration: none; }
...
/* TOPBAR */
.topbar { display: flex; align-items: center; gap: 14px; padding: 0 28px; height: 58px; background: var(--white); box-shadow: 0 1px 12px rgba(0,0,0,.07); flex-shrink: 0; z-index: 10; }
.search-wrap { flex: 1; display: flex; align-items: center; gap: 10px; background: #F4F6F5; border: 1.5px solid var(--border); border-radius: 9px; padding: 8px 14px; max-width: 420px; transition: border-color 0.2s; }
.search-wrap:focus-within { border-color: var(--green-light); }
.search-wrap input { border: none; outline: none; background: transparent; font-family: 'Poppins', sans-serif; font-size: 13px; color: var(--text-dark); width: 100%; }
.search-wrap input::placeholder { color: var(--text-light); }
.topbar-spacer { flex: 1; }
.notif-btn { position: relative; width: 38px; height: 38px; background: #F4F6F5; border: 1.5px solid var(--border); border-radius: 9px; display: flex; align-items: center; justify-content: center; cursor: pointer; }
.notif-dot { position: absolute; top: 7px; right: 8px; width: 7px; height: 7px; border-radius: 50%; background: #E53E3E; border: 1.5px solid var(--white); }
.admin-info { display: flex; align-items: center; gap: 10px; }
.admin-name { font-size: 13px; font-weight: 700; color: var(--text-dark); }
.admin-avatar { width: 36px; height: 36px; border-radius: 50%; background: linear-gradient(135deg, #52B788, #1B4332); display: flex; align-items: center; justify-content: center; font-size: 13px; font-weight: 800; color: #fff; flex-shrink: 0; }
...
.btn-nova { display: flex; align-items: center; gap: 7px; background: var(--green-mid); color: #fff; border: none; border-radius: 9px; padding: 10px 20px; font-family: 'Poppins', sans-serif; font-size: 13px; font-weight: 700; cursor: pointer; transition: background 0.2s; }
.btn-nova:hover { background: var(--green-dark); }

/* FILTER */
.filter-card { background: var(--white); border-radius: 16px; padding: 18px 20px 20px; margin-bottom: 18px; box-shadow: 0 1px 3px rgba(0,0,0,.05), 0 6px 20px rgba(0,0,0,.07); }
.filter-row { display: grid; grid-template-columns: 1fr 1fr 1fr auto; gap: 12px; align-items: end; }
.filter-group { display: flex; flex-direction: column; gap: 5px; }
.filter-group label { display: flex; align-items: center; gap: 5px; font-size: 11.5px; font-weight: 600; color: var(--text-gray); }
.filter-group input, .filter-group select { font-family: 'Poppins', sans-serif; font-size: 13px; color: var(--text-dark); background: var(--white); border: 1.5px solid var(--border); border-radius: 8px; padding: 9px 12px; outline: none; width: 100%; appearance: none; -webkit-appearance: none; transition: border-color 0.2s; }
.filter-group select { background-image: url("data:image/svg+xml,%3Csvg width='10' height='6' viewBox='0 0 10 6' fill='none' xmlns='http://www.w3.org/2000/svg'%3E%3Cpath d='M1 1l4 4 4-4' stroke='%238A9490' stroke-width='1.5' stroke-linecap='round' stroke-linejoin='round'/%3E%3C/svg%3E"); background-repeat: no-repeat; background-position: right 10px center; padding-right: 30px; }
.filter-group input:focus, .filter-group select:focus { border-color: var(--green-light); box-shadow: 0 0 0 3px rgba(82,183,136,.12); }
.filter-actions { display: flex; align-items: flex-end; }
.btn-limpar { display: inline-flex; align-items: center; gap: 6px; height: 39px; background: var(--white); color: var(--text-gray); border: 1.5px solid var(--border); border-radius: 8px; padding: 0 14px; font-family: 'Poppins', sans-serif; font-size: 12.5px; font-weight: 600; cursor: pointer; white-space: nowrap; transition: border-color 0.2s; }
.btn-limpar:hover { border-color: var(--text-gray); color: var(--text-dark); }

/* LOADING */
.loading-row { display: flex; align-items: center; gap: 10px; padding: 40px; font-size: 13px; color: var(--text-gray); justify-content: center; }
.spin { display: inline-block; width: 16px; height: 16px; border: 2px solid #C8D8CE; border-top-color: var(--green-mid); border-radius: 50%; animation: spin 0.7s linear infinite; }
.spin-sm { display: inline-block; width: 14px; height: 14px; border: 2px solid rgba(255,255,255,.35); border-top-color: #fff; border-radius: 50%; animation: spin 0.7s linear infinite; }
@keyframes spin { to { transform: rotate(360deg); } }

/* CATEGORY GRID */
.cat-grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(280px, 1fr)); gap: 16px; }
.cat-card { background: var(--white); border-radius: 16px; padding: 20px; box-shadow: 0 1px 3px rgba(0,0,0,.05), 0 6px 20px rgba(0,0,0,.07); transition: box-shadow 0.25s, transform 0.25s; display: flex; flex-direction: column; gap: 10px; }
.cat-card:hover { box-shadow: 0 4px 12px rgba(0,0,0,.1), 0 16px 40px rgba(0,0,0,.1); transform: translateY(-2px); }
.cat-card.inativa { opacity: 0.55; }
.cat-card-header { display: flex; align-items: center; gap: 10px; }
.cat-icon-wrap { width: 44px; height: 44px; border-radius: 12px; border: 1.5px solid; display: flex; align-items: center; justify-content: center; flex-shrink: 0; }
.cat-meta { flex: 1; }
.cat-code { font-size: 10.5px; font-weight: 700; letter-spacing: 1px; color: var(--text-light); text-transform: uppercase; background: #F4F6F5; border-radius: 5px; padding: 2px 7px; }
.cat-actions { display: flex; gap: 6px; }
.btn-icon-sm { width: 30px; height: 30px; border-radius: 7px; background: #F4F6F5; border: 1.5px solid var(--border); display: flex; align-items: center; justify-content: center; cursor: pointer; color: var(--text-gray); transition: all 0.15s; }
...
.tag { font-size: 11px; font-weight: 600; padding: 3px 9px; border-radius: 99px; background: var(--green-bg); color: var(--green-mid); border: 1px solid var(--green-pale); }

.cat-footer { display: flex; align-items: center; gap: 10px; margin-top: 4px; flex-wrap: wrap; }
.cat-stat { display: flex; align-items: center; gap: 5px; font-size: 12px; color: var(--text-light); font-weight: 600; }
.badge-ativa { font-size: 11px; font-weight: 700; padding: 3px 10px; border-radius: 99px; border: 1.5px solid; margin-left: auto; }
...
/* FOOTER */
.dash-footer { display: flex; align-items: center; justify-content: space-between; padding: 12px 28px; background: var(--white); box-shadow: 0 -1px 10px rgba(0,0,0,.06); font-size: 11.5px; color: var(--text-light); flex-shrink: 0; }
...
.modal { position: fixed; top: 50%; left: 50%; transform: translate(-50%, -50%); width: 560px; max-width: 95vw; max-height: 90vh; background: var(--white); z-index: 201; border-radius: 16px; overflow: hidden; display: flex; flex-direction: column; box-shadow: 0 20px 60px rgba(0,0,0,.18); }
.modal-subs { width: 500px; }
.modal-hd { display: flex; align-items: center; justify-content: space-between; padding: 20px 24px 18px; border-bottom: 1px solid var(--border); flex-shrink: 0; }
.modal-hd h3 { font-size: 16px; font-weight: 800; margin-bottom: 2px; }
.modal-hd p  { font-size: 12px; color: var(--text-light); }
.btn-close { width: 32px; height: 32px; background: #F4F6F5; border: 1.5px solid var(--border); border-radius: 8px; display: flex; align-items: center; justify-content: center; cursor: pointer; }
.btn-close:hover { background: #FFF5F5; border-color: #FC8181; }
.modal-body { flex: 1; overflow-y: auto; padding: 22px 24px; }
.modal-ft { display: flex; gap: 10px; justify-content: flex-end; padding: 16px 24px; border-top: 1px solid var(--border); flex-shrink: 0; }
...
.f-group input, .f-group select, .f-group textarea { font-family: 'Poppins', sans-serif; font-size: 13px; color: var(--text-dark); background: var(--white); border: 1.5px solid var(--border); border-radius: 9px; padding: 10px 13px; outline: none; width: 100%; appearance: none; -webkit-appearance: none; transition: border-color 0.2s, box-shadow 0.2s; }
.f-group select { background-image: url("data:image/svg+xml,%3Csvg width='10' height='6' viewBox='0 0 10 6' fill='none' xmlns='http://www.w3.org/2000/svg'%3E%3Cpath d='M1 1l4 4 4-4' stroke='%238A9490' stroke-width='1.5' stroke-linecap='round' stroke-linejoin='round'/%3E%3C/svg%3E"); background-repeat: no-repeat; background-position: right 13px center; padding-right: 36px; cursor: pointer; }
.f-group textarea { resize: vertical; min-height: 80px; }
.f-group input:focus, .f-group select:focus, .f-group textarea:focus { border-color: var(--green-light); box-shadow: 0 0 0 3px rgba(82,183,136,.12); }
.f-err { border-color: #FC8181 !important; }
.err-msg { font-size: 11.5px; color: #E53E3E; }
.req { color: #E53E3E; margin-left: 2px; }
.f-hint { font-size: 11px; color: var(--text-light); }
.f-hint-inline { font-size: 11px; color: var(--text-light); font-weight: 400; }
.api-error { background: #FFF5F5; border: 1px solid #FEB2B2; border-radius: 8px; padding: 10px 14px; font-size: 12.5px; color: #C53030; margin-bottom: 14px; }

.icon-preview { width: 48px; height: 48px; border-radius: 12px; border: 1.5px solid; display: flex; align-items: center; justify-content: center; }
...
.btn-cancelar { display: inline-flex; align-items: center; gap: 7px; background: transparent; color: var(--text-gray); border: 1.5px solid var(--border); border-radius: 9px; padding: 9px 20px; font-family: 'Poppins', sans-serif; font-size: 13px; font-weight: 600; cursor: pointer; }
.btn-cancelar:hover { border-color: var(--text-gray); color: var(--text-dark); }
.btn-guardar { display: inline-flex; align-items: center; gap: 7px; background: var(--green-mid); color: #fff; border: none; border-radius: 9px; padding: 10px 22px; font-family: 'Poppins', sans-serif; font-size: 13px; font-weight: 700; cursor: pointer; transition: background 0.2s; }
.btn-guardar:hover:not(:disabled) { background: var(--green-dark); }
.btn-guardar:disabled { opacity: 0.6; cursor: not-allowed; }

/* Subcategorias */
.sub-add-row { display: flex; gap: 10px; margin-bottom: 18px; }
.sub-add-row input { flex: 1; font-family: 'Poppins', sans-serif; font-size: 13px; color: var(--text-dark); border: 1.5px solid var(--border); border-radius: 9px; padding: 10px 13px; outline: none; transition: border-color 0.2s; }
.sub-add-row input:focus { border-color: var(--green-light); box-shadow: 0 0 0 3px rgba(82,183,136,.12); }
.btn-add-sub { display: inline-flex; align-items: center; gap: 7px; background: var(--green-mid); color: #fff; border: none; border-radius: 9px; padding: 10px 18px; font-family: 'Poppins', sans-serif; font-size: 13px; font-weight: 700; cursor: pointer; white-space: nowrap; transition: background 0.2s; }
.btn-add-sub:hover:not(:disabled) { background: var(--green-dark); }
.btn-add-sub:disabled { opacity: 0.6; cursor: not-allowed; }

.sub-list { display: flex; flex-direction: column; gap: 8px; }
.sub-item { display: flex; align-items: center; gap: 10px; background: #F7F9F8; border: 1px solid var(--border); border-radius: 9px; padding: 10px 14px; }
.sub-name { flex: 1; font-size: 13px; font-weight: 500; color: var(--text-dark); }
.sub-count { font-size: 11.5px; color: var(--text-light); white-space: nowrap; }
.sub-badge { margin-left: 0; flex-shrink: 0; }
.sub-edit-input { flex: 1; font-family: 'Poppins', sans-serif; font-size: 13px; color: var(--text-dark); border: 1.5px solid var(--green-light); border-radius: 7px; padding: 7px 10px; outline: none; box-shadow: 0 0 0 3px rgba(82,183,136,.12); }
.btn-sub-save { background: var(--green-mid); color: #fff; border: none; border-radius: 7px; padding: 6px 10px; font-size: 13px; font-weight: 700; cursor: pointer; }
.btn-sub-cancel { background: #F4F6F5; color: var(--text-gray); border: 1.5px solid var(--border); border-radius: 7px; padding: 6px 10px; font-size: 13px; cursor: pointer; }
.sub-empty { font-size: 13px; color: var(--text-light); font-style: italic; text-align: center; padding: 20px 0; }

/* TOAST */
.toast { position: fixed; bottom: 24px; right: 28px; z-index: 400; display: flex; align-items: center; gap: 10px; padding: 12px 20px; border-radius: 10px; font-family: 'Poppins', sans-serif; font-size: 13px; font-weight: 600; box-shadow: 0 8px 24px rgba(0,0,0,.15); }
...
.modal-pop-enter-active, .modal-pop-leave-active { transition: opacity 0.22s ease, transform 0.25s cubic-bezier(.4,0,.2,1); }
.modal-pop-enter-from, .modal-pop-leave-to { opacity: 0; transform: translate(-50%, -46%) scale(0.96); }
