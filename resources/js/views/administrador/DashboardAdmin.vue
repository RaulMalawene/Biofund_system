<template>
  <div class="app-shell">
      ...
      <main class="content">
        ...
        <div class="kpi-grid" :class="{ 'kpi-loading': statsLoading }">
          ...
          <div class="kpi-card">
            ...
            <div class="kpi-value dark">{{ statsLoading ? '—' : ((stats.totals.closed ?? 0) + (stats.totals.rejected ??
              0)) }}</div>
    ...
    <transition name="fade">
      <div class="submit-overlay" v-if="loading">
        <div class="submit-loader">
          <svg class="spin-lg" width="44" height="44" fill="none" stroke="#2D6A4F" stroke-width="2.5"
            viewBox="0 0 44 44">
            <circle cx="22" cy="22" r="18" stroke-opacity="0.15" />
            <path d="M22 4a18 18 0 0 1 18 18" stroke-linecap="round" />
          </svg>
          <p>A registar ocorrência…</p>
        </div>
      </div>
    </transition>
...
onMounted(async () => {
  ...
  barChart = new Chart(barChartRef.value, {
    ...
    data: {
      ...
      datasets: [{
        data: stats.byProvince.length ? stats.byProvince.map(p => p.total) : [0],
  ...
  donutChart = new Chart(donutChartRef.value, {
    ...
    data: {
      ...
      datasets: [{
        data: stats.byCategory.length ? stats.byCategory.map(c => c.total) : [1],
