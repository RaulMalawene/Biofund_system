          ...
          <div class="proj-card" v-for="p in filteredProjects" :key="p.id">
            <div class="card-top">
              <div class="card-icon-wrap" :style="{ background: iconColor(p.icon) + '22' }">
                <svg width="24" height="24" fill="none" :stroke="iconColor(p.icon)" stroke-width="1.7" viewBox="0 0 24 24" v-html="ICON_PATHS[p.icon || 'nature']"></svg>
              </div>
        ...
        <div class="notice" v-if="!loading">
          <div class="notice-icon">
            <svg width="16" height="16" fill="none" stroke="#8A9490" stroke-width="1.7" viewBox="0 0 16 16">
              <circle cx="8" cy="8" r="6"/><path d="M8 7v4M8 5.5h.01" stroke-linecap="round" stroke-width="1.8"/>
            </svg>
          </div>
          <div>
            <div class="notice-title">Nota sobre Taxonomia</div>
            <div class="notice-desc">A alteração do nome de um projecto irá actualizar automaticamente todos os registos históricos de reclamações associadas. Recomenda-se cautela ao desactivar projectos que possuam um elevado número de ocorrências.</div>
          </div>
        </div>
        ...
        <div class="modal-panel" @click.stop>
          <div class="modal-hd">
            <div>
              ...
              <p>{{ modal.editing ? 'Actualize os dados do projecto.' : 'Preencha os dados para criar um novo projecto.' }}</p>
            </div>
          ...
          <div class="modal-row">
            <div class="mfield">
              ...
              <input type="text" v-model="mf.code" placeholder="Ex: MOZ-001" maxlength="20" :class="{ 'f-err': fieldErrors.code }"/>
          ...
          <div class="mfield">
            ...
            <div class="icon-grid">
              <div
                class="icon-opt"
                :class="{ sel: mf.icon === key }"
                v-for="(path, key) in ICON_PATHS" :key="key"
                @click="mf.icon = key"
                :title="ICON_LABELS[key]"
              >
                <svg width="18" height="18" fill="none" :stroke="mf.icon === key ? iconColor(key) : '#8A9490'" stroke-width="1.7" viewBox="0 0 24 24" v-html="path"></svg>
              </div>
            </div>
          </div>
...
.search-wrap { flex: 1; display: flex; align-items: center; gap: 10px; background: #F4F6F5; border: 1.5px solid var(--border); border-radius: 9px; padding: 8px 14px; max-width: 420px; }
...
.breadcrumb { font-size: 11.5px; font-weight: 600; color: var(--green-mid); margin-bottom: 6px; text-transform: uppercase; letter-spacing: 0.5px; }
.page-title-row { display: flex; align-items: flex-start; justify-content: space-between; margin-bottom: 20px; }
.page-title-row h1 { font-size: 22px; font-weight: 800; margin-bottom: 5px; }
.page-title-row p  { font-size: 13px; color: var(--text-gray); max-width: 480px; line-height: 1.55; }
.btn-green-sm { display: inline-flex; align-items: center; gap: 7px; background: var(--green-mid); color: #fff; border: none; border-radius: 9px; padding: 11px 20px; font-family: 'Poppins', sans-serif; font-size: 13px; font-weight: 700; cursor: pointer; transition: background 0.2s; flex-shrink: 0; }
.btn-green-sm:hover { background: var(--green-dark); }

/* TOOLBAR */
.toolbar { display: flex; align-items: center; gap: 10px; margin-bottom: 20px; }
.search-proj { flex: 1; display: flex; align-items: center; gap: 9px; background: var(--white); border: 1.5px solid var(--border); border-radius: 9px; padding: 9px 14px; transition: border-color 0.2s; }
.search-proj:focus-within { border-color: var(--green-light); box-shadow: 0 0 0 3px rgba(82,183,136,.12); }
.search-proj input { border: none; outline: none; background: transparent; font-family: 'Poppins', sans-serif; font-size: 13px; color: var(--text-dark); width: 100%; }
.search-proj input::placeholder { color: var(--text-light); }
...
.spin-sm { display: inline-block; width: 13px; height: 13px; border: 2px solid rgba(255,255,255,.35); border-top-color: #fff; border-radius: 50%; animation: spin 0.7s linear infinite; }
@keyframes spin { to { transform: rotate(360deg); } }

/* PROJECT GRID */
.proj-grid { display: grid; grid-template-columns: repeat(3, 1fr); gap: 16px; margin-bottom: 20px; }

.proj-card { background: var(--white); border-radius: 16px; padding: 20px 20px 18px; display: flex; flex-direction: column; box-shadow: 0 1px 3px rgba(0,0,0,.05), 0 6px 20px rgba(0,0,0,.07); transition: box-shadow 0.2s, transform 0.2s; }
.proj-card:hover { box-shadow: 0 6px 24px rgba(0,0,0,.12); transform: translateY(-2px); }

.card-top { display: flex; align-items: flex-start; justify-content: space-between; margin-bottom: 14px; }
.card-icon-wrap { width: 46px; height: 46px; border-radius: 12px; display: flex; align-items: center; justify-content: center; flex-shrink: 0; }
.card-top-right { display: flex; flex-direction: column; align-items: flex-end; gap: 5px; }

.proj-code { font-size: 10px; font-weight: 700; letter-spacing: 1px; color: var(--text-light); text-transform: uppercase; background: #F4F6F5; border-radius: 5px; padding: 2px 7px; }

.badge-ativa  { font-size: 11px; font-weight: 700; color: var(--green-mid); background: var(--green-pale); border: 1.5px solid #68D391; border-radius: 99px; padding: 2px 10px; }
.badge-inativa{ font-size: 11px; font-weight: 700; color: #744210; background: #FEFCBF; border: 1.5px solid #F6D860; border-radius: 99px; padding: 2px 10px; }

.proj-name { font-size: 16px; font-weight: 800; margin-bottom: 6px; color: var(--text-dark); }
.proj-desc { font-size: 12px; color: var(--text-gray); line-height: 1.6; flex: 1; margin-bottom: 16px; }
...
.occ-label { font-size: 9.5px; font-weight: 700; letter-spacing: 0.7px; color: var(--text-light); text-transform: uppercase; margin-bottom: 2px; }
.occ-val   { font-size: 20px; font-weight: 800; color: var(--text-dark); }

.card-actions { display: flex; gap: 8px; }
.btn-card-action { width: 30px; height: 30px; border-radius: 7px; border: none; display: flex; align-items: center; justify-content: center; cursor: pointer; transition: background 0.15s; }
.btn-card-edit { background: var(--green-bg); color: var(--green-mid); }
.btn-card-edit:hover { background: var(--green-pale); }

.new-proj-card { background: var(--white); border: 2px dashed var(--border); border-radius: 16px; display: flex; flex-direction: column; align-items: center; justify-content: center; gap: 12px; cursor: pointer; padding: 40px 20px; transition: border-color 0.2s, background 0.2s; min-height: 200px; }
.new-proj-card:hover { border-color: var(--green-light); background: var(--green-bg); }
.new-proj-icon { width: 44px; height: 44px; border-radius: 50%; background: #F4F6F5; border: 1.5px solid var(--border); display: flex; align-items: center; justify-content: center; transition: background 0.2s, border-color 0.2s; }
.new-proj-card:hover .new-proj-icon { background: var(--green-pale); border-color: var(--green-light); }
.new-proj-label { font-size: 13px; font-weight: 600; color: var(--text-gray); }
.new-proj-card:hover .new-proj-label { color: var(--green-mid); }

/* NOTICE */
.notice { display: flex; align-items: flex-start; gap: 12px; background: var(--white); border-radius: 16px; padding: 16px 18px; margin-top: 6px; box-shadow: 0 1px 3px rgba(0,0,0,.05), 0 6px 20px rgba(0,0,0,.07); }
.notice-icon { width: 34px; height: 34px; border-radius: 8px; background: #F4F6F5; display: flex; align-items: center; justify-content: center; flex-shrink: 0; margin-top: 1px; }
.notice-title { font-size: 13px; font-weight: 700; margin-bottom: 4px; }
.notice-desc  { font-size: 12.5px; color: var(--text-gray); line-height: 1.65; }
...
.modal-panel { background: var(--white); border-radius: 16px; padding: 30px 32px 26px; width: 500px; max-width: 95vw; box-shadow: 0 16px 60px rgba(0,0,0,.18); }
...
.modal-hd p  { font-size: 12.5px; color: var(--text-gray); margin-top: 3px; }
...
.modal-row { display: grid; grid-template-columns: 1fr 1fr; gap: 12px; margin-bottom: 0; }
.mfield { display: flex; flex-direction: column; gap: 6px; margin-bottom: 16px; }
.mfield label { font-size: 12.5px; font-weight: 600; color: var(--text-dark); }
.mfield input, .mfield select, .mfield textarea { font-family: 'Poppins', sans-serif; font-size: 13px; color: var(--text-dark); border: 1.5px solid var(--border); border-radius: 8px; padding: 10px 13px; outline: none; transition: border-color 0.2s, box-shadow 0.2s; width: 100%; background: var(--white); }
.mfield textarea { min-height: 80px; resize: vertical; }
.mfield input:focus, .mfield select:focus, .mfield textarea:focus { border-color: var(--green-light); box-shadow: 0 0 0 3px rgba(82,183,136,.12); }
...
.icon-opt { width: 40px; height: 40px; border-radius: 9px; background: #F4F6F5; border: 2px solid var(--border); display: flex; align-items: center; justify-content: center; cursor: pointer; transition: background 0.15s, border-color 0.15s; }
.icon-opt:hover, .icon-opt.sel { background: var(--green-bg); border-color: var(--green-mid); }

.modal-actions { display: flex; gap: 10px; margin-top: 8px; }
.btn-mc { flex: 1; background: var(--white); color: var(--text-gray); border: 1.5px solid var(--border); border-radius: 9px; padding: 11px; font-family: 'Poppins', sans-serif; font-size: 13px; font-weight: 600; cursor: pointer; }
.btn-mc:hover { border-color: var(--text-gray); }
.btn-ms { flex: 1; background: var(--green-mid); color: #fff; border: none; border-radius: 9px; padding: 11px; font-family: 'Poppins', sans-serif; font-size: 13px; font-weight: 700; cursor: pointer; display: flex; align-items: center; justify-content: center; gap: 8px; transition: background 0.2s; }
.btn-ms:hover:not(:disabled) { background: var(--green-dark); }
.btn-ms:disabled { background: #A0C4B0; cursor: not-allowed; }

/* TOAST */
.toast { position: fixed; bottom: 24px; right: 24px; z-index: 500; background: var(--green-dark); color: #fff; border-radius: 12px; padding: 13px 18px; display: flex; align-items: center; gap: 9px; font-size: 13px; font-weight: 600; box-shadow: 0 8px 28px rgba(0,0,0,.18); }
