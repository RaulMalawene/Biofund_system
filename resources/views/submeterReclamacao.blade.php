...
<head>
  ...
  <title>BioQueixa – Registar Reclamação</title>
  ...
  <style>
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .page-header p { font-size: 14px; color: var(--text-gray); line-height: 1.7; max-width: 520px; margin-bottom: 36px; }
    ...
    .form-card:nth-child(3){animation-delay:.1s}
    ...
    .field-group input::placeholder, .field-group textarea::placeholder { color: var(--text-light); }
    .field-group input:focus, .field-group select:focus, .field-group textarea:focus {
      border-color: var(--green-light); box-shadow: 0 0 0 3px rgba(82,183,136,.18);
    }
    ...
    .select-wrap::after {
      ...
      width: 10px; height: 6px; background: var(--text-light);
      clip-path: polygon(0 0,100% 0,50% 100%); pointer-events: none;
    }
    .date-wrap { position: relative; }
    .date-wrap input { padding-left: 40px; }
    .date-icon { position: absolute; left: 13px; top: 50%; transform: translateY(-50%); pointer-events: none; }
    ...
    .upload-icon {
      ...
      border-radius: 12px; display: flex; align-items: center; justify-content: center; margin: 0 auto 16px;
    }
    ...
    .file-item {
      ...
      background: var(--green-bg); border: 1px solid #C3E6CE;
    ...
    .file-remove { background: none; border: none; cursor: pointer; color: var(--text-light); display: flex; transition: color .2s; }
    ...
    .submit-banner-icon {
      ...
      border-radius: 8px; display: flex; align-items: center; justify-content: center; flex-shrink: 0; margin-top: 1px;
    }
    .submit-banner p { font-size: 12.5px; color: rgba(255,255,255,.85); line-height: 1.65; max-width: 440px; }
    .btn-submit {
      display: inline-flex; align-items: center; gap: 8px;
      background: var(--green-light); color: var(--white); border: none;
      border-radius: 9px; padding: 13px 28px;
      font-family: 'Poppins',sans-serif; font-size: 14px; font-weight: 700;
      cursor: pointer; white-space: nowrap; flex-shrink: 0; transition: background .2s, transform .15s;
    }
    ...
    .success-card {
      background: var(--white); border-radius: 18px; padding: 48px 40px;
    ...
    .footer-brand p { font-size: 12.5px; color: var(--text-gray); line-height: 1.65; max-width: 220px; }
...
<div id="app">
  ...
  <nav>
    ...
    <div class="nav-links">
      <a href="/">Inicio</a>
      <a href="/submeterReclamacao" style="color:var(--green-mid);font-weight:700">Submeter</a>
        ...
        <div class="field-group">
          ...
          <div class="select-wrap">
            <select v-model="form.categoria">
              ...
              <option>Tráfico de Animais</option>
              <option>Degradação de Habitat</option>
              <option>Outro</option>
            </select>
          </div>
        </div>
      ...
      <div class="field-row single">
        <div class="field-group">
          ...
          <input type="text" v-model="form.coordenadas"
            placeholder="Ex: -25.9682, 32.5732 ou descrição do local (ex: 'Próximo à escola primária')" />
        </div>
      </div>
    </div>
...
createApp({
  setup() {
    ...
    const submitForm = () => {
      if(!form.contacto){ alert('Por favor, preencha o campo Telefone ou Email.'); return; }
      showSuccess.value = true;
    };

    return { form, files, isDragging, showSuccess, fileInput,
             provincias, distritos, comunidades, triggerUpload, handleFileSelect,
             handleDrop, removeFile, submitForm };
  }
}).mount('#app');
</script>
</body>
